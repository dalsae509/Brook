...
**자동 입찰 시나리오** (서로 다른 두 입찰자 계정)
1. 시작가 50,000원 경매에서 A가 자동 입찰 최대가 **100,000원** 설정 → 시작가로 선점
2. B가 자동 입찰 최대가 **150,000원** 설정 → B가 **110,000원**(A 최대가 + 한 단위)에 자동 선점
3. B는 150,000원을 다 내지 않고 필요한 만큼만 입찰 — 밀려난 A에게 자동 입찰 알림 발송
...
- 관리자 대시보드 (사용자·상품·신고 관리, 경매 강제 종료)
...
`node:test` + `supertest` + `mongodb-memory-server`(레플리카셋, 트랜잭션 지원)로 인증·거래 완료·자동 입찰·시세·대시보드 등 핵심 플로우를 테스트합니다. GitHub Actions가 push/PR마다 클라이언트 lint·build와 서버 테스트·문법 검사를 실행해, 깨진 코드가 배포되는 것을 차단합니다.
