...
- **판매자 통계 대시보드** — 매출·조회수·상태 분포·월별 매출·인기 상품 차트
...
| 차트 | 직접 구현 (HTML/CSS, 외부 라이브러리 미사용) |
...
**동시성 스트레스 테스트로 실증**: 서로 다른 50명이 동일가로 **동시 입찰**했을 때 **정확히 1건만 낙찰**되고 입찰 레코드도 1건만 생성됨(중복 낙찰·갱신 유실 0건). 30명이 상승가로 동시 입찰해도 최종 현재가가 최고 입찰가와 일치(로스트 업데이트 0건).
...
`node:test` + `supertest` + `mongodb-memory-server`(레플리카셋, 트랜잭션 지원)로 인증·거래 완료·자동 입찰·**동시성**·채팅·브룩 지수·신고·시세·대시보드·검색·추천 등 핵심 플로우를 **43개** 테스트로 검증합니다. GitHub Actions가 push/PR마다 클라이언트 lint·build와 서버 테스트·문법 검사를 실행해, 깨진 코드가 배포되는 것을 차단합니다.

**시험 결과 요약** (`npm run test:coverage`)

| 항목 | 결과 |
|------|------|
| 자동화 테스트 | **43개 전부 통과** |
| 동시 입찰 50건 동시 발생 | **1건만 낙찰** (중복 낙찰·갱신 유실 0건) |
| 전체 커버리지 | 라인 **56%** / 브랜치 **62%** |
| 핵심 로직 커버리지 | `bidUnit`(입찰 단위) **100%**, `proxyBidUtil`(자동 입찰) **100%**, `brookScoreUtil`(신뢰도) **85%** |
